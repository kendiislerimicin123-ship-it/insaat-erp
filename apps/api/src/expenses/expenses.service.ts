...
export class ExpensesService {
  ...
  async update(tenantId: string, userId: string, id: string, dto: UpdateExpenseDto) {
    ...
    if (existing.isAutoGenerated) {
      throw new BadRequestException(
        `Bu gider otomatik üretildi (${existing.sourceType}). Manuel düzenleme yapılamaz. Değişiklik için kaynak kaydı (hakediş/cari hareketi/puantaj/stok hareketi) güncelleyin.`,
      );
    }
  ...
  async remove(tenantId: string, userId: string, id: string) {
    ...
    if (expense.isAutoGenerated) {
      throw new BadRequestException(
        `Bu gider otomatik üretildi (${expense.sourceType}). Manuel silinemez. Kaynak kaydı (hakediş/cari hareketi/puantaj/stok hareketi) iptal edilirse otomatik silinir.`,
      );
    }
  ...
  // ════════════════════════════════════
  // MALZEME HAREKETİ → EXPENSE
  // Kural (D): sadece type=IN + unitPrice var
  // Proje seçiliyse projeye, seçilmemişse genel gider
  // ════════════════════════════════════
  async createFromMaterialMovement(tenantId: string, movementId: string) {
    try {
      ...
      // Fiyatsız giriş = tutar bilinmiyor, çöp kayıt üretme
      if (!movement.totalPrice || new Prisma.Decimal(movement.totalPrice).lte(0)) {
        this.logger.warn(`⚠️  Stok girişinde fiyat yok/sıfır, expense üretilmiyor: ${movementId}`);
        return null;
      }

      if (await this.existsFromSource(tenantId, 'MATERIAL_MOVEMENT', movementId)) {
        this.logger.log(`ℹ️  Bu stok hareketi için zaten expense var, atlanıyor: ${movementId}`);
        return null;
      }

      const code = await this.generateAutoCode(tenantId);
      const qty = new Prisma.Decimal(movement.quantity).toString();
      const description = `Malzeme alımı - ${movement.material?.name ?? 'Bilinmiyor'} ${qty} ${movement.material?.unit ?? ''}${movement.project ? ` - ${movement.project.name}` : ' (Depo)'}`;

      const totalPrice = new Prisma.Decimal(movement.totalPrice);
      ...
      const expense = await this.prisma.expense.create({
        data: { 
          ...
          sourceId: movementId,
          isAutoGenerated: true,
          notes: `Kaynak: Stok hareketi ${movementId.substring(0, 8)}${movement.supplier ? ` | Tedarikçi: ${movement.supplier}` : ''}`,
          createdBy: 'SYSTEM',
          updatedBy: 'SYSTEM',
        },
      });

      this.logger.log(`🤖 OTO Gider üretildi (Malzeme): ${expense.code} ← ${movement.material?.code} ${qty} ${movement.material?.unit} (${totalPrice.toString()} ${movement.currency})`);

      return expense;
    } catch (error) {
      this.logger.error(`❌ createFromMaterialMovement hatası: ${error instanceof Error ? error.message : String(error)}`);
      return null;
    }
  }
