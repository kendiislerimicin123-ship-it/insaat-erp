...
export default function ExpensesPage() {
  ...
  const hasActiveFilters =
    search || categoryFilter || statusFilter || projectFilter || fromDate || toDate || sourceFilter !== 'ALL';

  // Kaynak filtresi client-side uygulanır (backend'de query param yok)
  const visibleItems = items.filter((e) => {
    if (sourceFilter === 'AUTO') return e.isAutoGenerated;
    if (sourceFilter === 'MANUAL') return !e.isAutoGenerated;
    return true;
  });

  const autoCount = items.filter((e) => e.isAutoGenerated).length;
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-600 mt-1">
            Manuel giderler + sistem tarafından otomatik üretilen kayıtlar
          </p>
        </div>
        {mounted && canCreate && (
          <button
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4">
          ...
          <SummaryCard
            ...
            value={`${autoCount}`}
            sub={`Bu sayfada ${items.length} kayıttan`}
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 mb-4">
        ...
        <div className="flex items-center gap-1 mb-3 bg-slate-100 rounded-lg p-1 w-fit">
          {([
            ...
            ['MANUAL', '✋ Manuel'],
            ['AUTO', '🤖 Otomatik'],
          ] as Array<['ALL' | 'MANUAL' | 'AUTO', string]>).map(([key, label]) => (
            <button
              key={key}
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center">
            ...
            {mounted && canCreate && !hasActiveFilters && (
              <button
                onClick={() => {
                  setEditItem(null);
                  setFormOpen(true);
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {visibleItems.map((e) => (
                  <tr
                    key={e.id}
                    className={`hover:bg-slate-50 ${e.isAutoGenerated ? 'bg-purple-50/30' : ''}`}
                  >
                    <td className="px-3 py-2">
                      <div className="flex items-center gap-1.5">
                        {e.isAutoGenerated && (
                          <span
                            className="text-xs"
                            title={`Otomatik: ${e.sourceType ? EXPENSE_SOURCE_LABELS[e.sourceType] : ''}`}
                          >
                            🔗
                          </span>
                        )}
                    ...
                    <td className="px-3 py-2">
                      <span
                        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium ${
                          EXPENSE_CATEGORY_COLORS[e.category] ?? 'bg-slate-100 text-slate-700'
                        }`}
                      >
                        <span>{EXPENSE_CATEGORY_ICONS[e.category] ?? '❓'}</span>
                        <span>{EXPENSE_CATEGORY_LABELS[e.category] ?? e.category}</span>
                      </span>
                      {e.isAutoGenerated && e.sourceType && (
                        <p className="text-[10px] text-purple-700 mt-1 font-medium">
                          {EXPENSE_SOURCE_LABELS[e.sourceType]}&apos;den
                        </p>
                      )}
                    </td>
                    <td className="px-3 py-2 max-w-xs">
                      <p className="text-slate-900 truncate" title={e.description}>
                        {e.description}
                      </p>
                      {e.invoiceNo && (
                    ...
                    <td className="px-3 py-2 text-right whitespace-nowrap">
                      {e.isAutoGenerated ? (
                        <span
                          ...
                          title="Otomatik kayıtlar buradan düzenlenemez. Kaynak kaydı değiştirin."
                        >
                          Kilitli
                        </span>
                      ) : (
